...
export async function generateNextState(playerInput, currentStateData) {
  ...
  if (!apiKey) {
    throw new Error("Missing Gemini API Key. Please provide an API key in the Setup section.");
  }
...
Inventory: ${currentStateData.inventory.join(', ')}
...
Staff Turnover: ${currentStateData.staffTurnover}\n\n`;
  ...
  try {
    ...
    // Parse the JSON. We might need to clean it if the model wrapped it anyway.
